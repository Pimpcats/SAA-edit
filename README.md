...
- **Launch backends headlessly (no webpage):** the Video tab has a **Local servers (launch headless)** panel where you point the app at your existing ComfyUI (`run_nvidia_gpu.bat`) and A1111 (`webui-user.bat`) launch scripts. The app starts the **server only** — it never opens a browser page — then waits until the API responds. Optional **auto-start with app**, and the servers it launched are shut down when you close the app. Always-visible **"Comfy Ready" / "A1111 Ready"** badges (top-right) turn green once each backend's API is reachable. For A1111 make sure your `webui-user.bat` has `--api` and **not** `--autolaunch`; ComfyUI's standalone server doesn't open a browser unless you pass `--auto-launch`.
